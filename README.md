...
_Process mining with LLM analysis_
...
The application works with event logs in **CSV** or **XES** format. The `dataset/examples` directory contains **example datasets** so the pipeline can be run out of the box.

Event logs are expected to provide at least three fields, mapped in the configuration through `dataset.columns`. Place any dataset you want to analyze inside the `dataset/` directory and reference it from `config.json`.
...
|---|---|
| `app/` | Core process mining application (pm4py) and the API server. Discovers models, runs the LLM analysis, simplifies DFGs and renders diagrams. |
| `orchestrator/` | Client API that coordinates the full scheduled pipeline across services. |
| `moodle-data-service/` | Service (with a Redis/RQ worker) that fetches data from a Moodle instance to build datasets. |
| `mermaid-service/` | Renders the Mermaid diagrams generated by the application into images. |
| `results-publisher/` | Publishes the analysis results to a configured destination. |
...
This section describes the end-to-end analysis pipeline and how to run it through the orchestrator container.
