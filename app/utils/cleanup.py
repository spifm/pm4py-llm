"""
Cleanup utility for old output directories.

Removes subdirectories of /output whose name is exactly YYYYMMDD
and whose date is <= the given cutoff date. Directories with any other
naming format are left untouched.

By default runs in dry-run mode (prints what would be deleted).
Pass --apply to actually delete.

Usage:
  python cleanup.py 20251231
  python cleanup.py 20251231 --apply
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Delete output subdirectories named YYYYMMDD whose date is on or before "
...
def main() -> None:
    ...
    if not OUTPUT_ROOT.is_dir():
        print(f"Error: output root '{OUTPUT_ROOT}' does not exist or is not a directory.", file=sys.stderr)
        sys.exit(1)
