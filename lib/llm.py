...
def analyze_petri_net(client, llm_config, net, im, fm, file_name):
    print("\n\n-------------------\nPetri net analysis\n-------------------\n\n")
    ...
    exec_prompt(client, llm_config['petri_net'], prompt, file_name)

# Analyze abstract Directly-Follows Graph model
def analyze_dfg(client, llm_config, filtered_log, file_name):
    print("\n\n-------------\nDFG analysis\n------------\n\n")
    dfg_description = pm4py.llm.abstract_dfg(filtered_log)
    prompt = f"{llm_config['context']}\n\nHere is the DFG model:\n\n{dfg_description}"
    exec_prompt(client, llm_config['dfg'], prompt, file_name)

# Analyze abstract Temporal Profile
def analyze_temporal_profile(client, llm_config, temporal_profile, file_name):
    ...
    exec_prompt(client, llm_config['temporal_profile'], prompt, file_name)

# Export filtered log
def exec_prompt(client, llm_config_model, prompt, output_file):
