...
if __name__ == "__main__":
    ...
    filter_game_result = config['filter']['attr']['game_result']
    filter_exam_result = config['filter']['attr']['exam_result']
    filter_grade_er = config['filter']['attr']['grade_er']
    ...
    if filter_game_result > 0:
        filter_param, min, max, filtered_log = filter.filter_log_by_game_outcome(log, filter_level)
   
    # Filter by exam result
    if filter_exam_result > 0:
        filter_param, min, max, filtered_log = filter.filter_log_by_exam_result(log, filter_level)

    # Filter by grade er
    if filter_grade_er > 0:
        filter_param, min, max, filtered_log = filter.filter_log_by_grade_er(log, filter_level)
    ...
    if debug > 0:
        if filter_level == "event":
            ...
            trace_values = pm4py.stats.get_trace_attribute_values(filtered_log, 'case:' + filter_param)
            print("\nTrace values ({}): {}".format(filter_param, trace_values))
